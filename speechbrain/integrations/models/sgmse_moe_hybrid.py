"""
Hybrid Mixture of Experts for SGMSE.

This module implements hybrid routing combining both timestep-based and noise-based routing.
Routes experts based on BOTH diffusion timestep t AND environmental noise characteristics (SNR, noise type).
...
class ScoreModelMoEHybrid(nn.Module):
    """
    Score-based generative model with Hybrid Mixture of Experts.
    
    Creates N experts from K basis models using learnable mixing coefficients.
    Routes to experts based on BOTH:
    1. Diffusion timestep t (timestep-based routing)
    2. Environmental noise characteristics (SNR, noise type) (noise-based routing)
    ...
    - Hybrid routing: combines timestep and noise characteristics
    ...
    num_experts: int
        Number of expert models to create via mixing (default: 100).
        Should be >= K. Typically: num_timestep_bins * num_snr_bins * num_noise_types
    num_timestep_bins: int
        Number of bins to discretize timestep t into (default: 10).
    num_snr_bins: int
    ...
    routing_strategy: str
        How to combine timestep and noise routing:
        - "grid": expert_id = (t_bin * num_snr_bins * num_noise_types) + (snr_bin * num_noise_types) + noise_type_id
        - "hierarchical": First route by noise, then by timestep
        - "learned": Use learned combination weights (future work)
        Default: "grid"
    lr: float
    ...
    **kwargs
    ...
    def __init__(
        ...
        num_experts=100,  # Number of experts to create
        ...
        routing_strategy="grid",  # How to combine routing dimensions
        lr=1e-4,
        ema_decay=0.999,
        t_eps=0.03,
        num_eval_files=20,
        ...
        **kwargs,
        ...
        # Expected number of experts for full coverage
        expected_experts = num_timestep_bins * num_snr_bins * num_noise_types
        if num_experts < expected_experts:
            print(
                f"Warning: num_experts ({num_experts}) < expected ({expected_experts}) "
                f"for full coverage. Some combinations will share experts."
            )
    ...
    def _get_expert_id(self, t, snr_db, noise_type_id):
        """
        Map (timestep bin, SNR bin, noise type) to expert ID using hybrid routing.
        ...
        """
        t_bin = self._get_timestep_bin(t)
        snr_bin = self._get_snr_bin(snr_db)
        
        if self.routing_strategy == "grid":
            # Grid-based: expert_id = (t_bin * num_snr_bins * num_noise_types) + 
            #                        (snr_bin * num_noise_types) + noise_type_id
            expert_ids = (
                t_bin * self.num_snr_bins * self.num_noise_types +
                snr_bin * self.num_noise_types +
                noise_type_id
            )
        elif self.routing_strategy == "hierarchical":
            # Hierarchical: First by noise (snr_bin, noise_type), then by timestep
            noise_expert = snr_bin * self.num_noise_types + noise_type_id
            expert_ids = noise_expert * self.num_timestep_bins + t_bin
        else:
            raise ValueError(f"Unknown routing_strategy: {self.routing_strategy}")
        
        # If num_experts < expected, use modulo to map to available experts
        if self.num_experts < (self.num_timestep_bins * self.num_snr_bins * self.num_noise_types):
    ...
    def forward(self, x_t, y, t, x=None, snr_db=None, noise_type_id=None):
        """
        Computes the score or predicted clean data using hybrid MoE routing.
        ...
        # Route to expert based on BOTH timestep and noise characteristics
        expert_id = self._get_expert_id(t, snr_db, noise_type_id)
    ...
    def enhance(
        ...
        **kwargs,
        ...
        """
        One-call speech enhancement from a noisy input using hybrid MoE.
        
        This method runs the chosen SGMSE sampler to produce an enhanced spectrogram
        from the input `y`, using hybrid expert routing.
        """
